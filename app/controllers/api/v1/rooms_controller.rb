class Api::V1::RoomsController < ApplicationController
    ...
    def index
        ...
        render json: {
            rooms: rooms.map {|room| room.attributes.merge(image: rails_blob_url(room.cover_photo), instant: room.instant != "Request")},
            is_success: true
        }, status: :ok
    end
    ...
    def show
        ...
        if !room.nil?
            room_serializer = RoomSerializer.new(
                room,
                image: rails_blob_url(room.cover_photo),
    ...
    def your_listings
        ...
        render json: {
            rooms: rooms.map {|r| r.attributes.merge(image: rails_blob_url(r.cover_photo), instant: r.instant != "Request")},
            is_success: true
        }, status: :ok
    end
end